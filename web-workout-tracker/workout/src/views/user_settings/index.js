...
class UserSettings extends React.Component {
    ...
    update_active_setting = e => this.setState({setting_selected: e.currentTarget.id})
    update_body_type = e => {
        if(this.state.setting_selected === 'ideal-body-type') {
            e.stopPropagation()
            this.setState({ideal_body_type: e.currentTarget.id, setting_selected: ''})
        }
    }
    render = () =>
        <SECTION className='user-settings'>
            <div id='user-avatar' className='field' onClick={this.update_active_setting}>
                <span className='title'>That's You!</span>
                <div className='value'>
                    <img src='/' alt='you!'/>
                </div>
            </div>
            <div id='first-name' className='field' onClick={this.update_active_setting}>
                <span className='title'>First Name</span>
                <div className='value'>
                    <input type='text' id='first_name' value={this.state.first_name} onChange={this.update_field} />
                </div>
            </div>
            <div id='last-name' className='field' onClick={this.update_active_setting}>
                <span className='title'>First Name</span>
                <div className='value'>
                    <input type='text' id='last_name' value={this.state.last_name} onChange={this.update_field} />
                </div>
            </div>
            <div id='ideal-body-type' className='field' onClick={this.update_active_setting}>
                <span className='title'>Ideal Body Type</span>
                <div className='value' >
                    {this.state.body_types.filter(type => this.state.setting_selected === 'ideal-body-type' ? true : type.name === this.state.ideal_body_type).map((type, idx) => (
                        <div className='type' key={idx} id={type.name} onClick={this.update_body_type}>
            ...
            <div id='units-of-measurment' className='field' onClick={this.update_active_setting}>
